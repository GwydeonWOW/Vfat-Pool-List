...
export function calculateRiskScores(pool, history = [], now = Date.now(), capital = DEFAULT_CAPITAL, target = DAILY_PROFIT_TARGET) {
  ...
  const isConcentratedFarm = /^CL\d+-/i.test(String(pool.vfname || ''));
  ...
  for (const [profile, assumptions] of Object.entries(PROFILE_ASSUMPTIONS)) {
    ...
    const concentratedFarmBonus = isConcentratedFarm ? (meetsTarget ? 12 : 5) : 0;
    const adjustedScore = opportunityScore - liquidityPenalty + concentratedFarmBonus;
    ...
    scores[profile] = {
      ...
      meetsTarget,
      isConcentratedFarm,
      concentratedFarmBonus,
      confidence: Number(dataConfidence.toFixed(1)),
      provisional: historyDays < 1,
      components: { opportunity: Number(opportunityScore.toFixed(1)), concentratedFarmBonus, liquidityPenalty: Number(liquidityPenalty.toFixed(1)), realization: assumptions.realization * 100, positionShare: Number((estimatedPositionShare * 100).toFixed(2)) },
