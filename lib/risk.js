...
export function calculateRiskScores(pool, history = [], now = Date.now(), capital = DEFAULT_CAPITAL, target = DAILY_PROFIT_TARGET) {
  ...
  const capacityLimitedGross = poolRewardsDaily == null ? aprGrossDaily : Math.min(aprGrossDaily, poolRewardsDaily);
  ...
  for (const [profile, assumptions] of Object.entries(PROFILE_ASSUMPTIONS)) {
    ...
    const opportunityScore = meetsTarget
      ? 50 + clamp((netDaily - dailyTarget) / dailyTarget * 40, 0, 40) + dataConfidence / 10
      : clamp(netDaily / dailyTarget * 49, 0, 49);
    const total = clamp(opportunityScore - liquidityPenalty);
    ...
    scores[profile] = {
      ...
      components: { opportunity: Number(opportunityScore.toFixed(1)), liquidityPenalty: Number(liquidityPenalty.toFixed(1)), realization: assumptions.realization * 100 },
