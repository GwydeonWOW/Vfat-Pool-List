...
const rows = [
  ...
  ['Pool fees/day', p => p._risk ? formatUsd(p._risk.poolFeesDaily) : '-'],
  ['Pool incentives/day', p => p._risk ? `${formatUsd(p._risk.poolIncentivesDaily)}${p._risk.incentiveDependent ? ' (incentives only)' : ''}` : '-'],
  ['Estimated reward share', p => p._risk ? `${p._risk.estimatedPositionShare}%` : '-'],
