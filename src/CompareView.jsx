...
const rows = [
  ['Risk score', p => p._risk?.total ?? '-'], ['Confidence', p => p._risk ? `${p._risk.confidence}%` : '-'],
...
export default function CompareView({ poolIds, profile, onRemove }) {
  ...
  return <section className="compare-view"><h2>Pool comparison · {profile}</h2>{error && <div className="error">{error}</div>}
    ...
    {pools.map(p => p._risk && <details key={p.id} className="risk-details"><summary>{p.pair}: risk breakdown</summary>{Object.entries(p._risk.components).map(([k,v]) => <span key={k}>{k}: {Number(v).toFixed(1)}</span>)}</details>)}
  </section>;
}
